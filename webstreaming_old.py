...
#from pyimagesearch.color_sensor import arduinoColorSensor #as cs
...
from requests_ntlm import HttpNtlmAuth #modify checkVersion() #ethan
import json
#######################################################
...
def checkVersion():
    ...
    try:
        ...
        r = requests.get(url, timeout=0.5,auth=HttpNtlmAuth("DCT_A32","@@88888888oo")) #ethan
    ...
    if r.status_code == requests.codes.ok:
        print("Server OK")
        j = json.loads(r.text) #ethan
        latest_version = j["current_version"]
    else:
        # Can not connect server
...
def detect_color():
    ...
    while True:
        ...
        if not iscalibrate and not TKT.iscalibrate:
            ...
            #print("red_array:",type(red_array)," ",red_array)
            #print("red:",type(red)," ",red)
...
def correctPidate(user,pw,url):  #ethan
    # Get the latest version from dct server
    try:
        r = requests.get(url, timeout=0.5,auth=HttpNtlmAuth(user,pw))
        j=json.loads(r.text)
        date=j["NOW"]
        print(date)
        os.system("sudo date "+date)
        printDebugLog("Correct date OK")
    except:
        printDebugLog("Correct date NG")
# Website function
@app.route("/")
def index():
...
if __name__ == '__main__':
    try:
        ...
        try: #add by ethan
            #correctPidate("DCT_A32","@@88888888oo","http://192.168.0.10/dct_a32/api/ClientSvc/getNow")
            rv = checkVersion()
            ...
            while rv:
                ...
                printDebugLog("checkVersion Error")
